...
import { APIPromise } from 'gentrace/core/api-promise';

import util from 'node:util';
import Gentrace from 'gentrace';
import { APIUserAbortError } from 'gentrace';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['GENTRACE_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: Gentrace) => {
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['GENTRACE_BASE_URL'] = undefined;
    });
    ...
    test('in request options overridden by client options', () => {
      const client = new Gentrace({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
    ...
    test('in request options overridden by env variable', () => {
      process.env['GENTRACE_BASE_URL'] = 'http://localhost:5000/env';
      const client = new Gentrace({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
